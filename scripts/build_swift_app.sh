...
# Build signed macOS .app for Gelato (Swift)
# Usage:
...
BINARY_PATH=".build/release/Gelato"
...
# Copy binary
cp "$BINARY_PATH" "$APP_DIR/Contents/MacOS/Gelato"
...
APP_BINARY="$APP_DIR/Contents/MacOS/Gelato"
...
# Copy Info.plist
cp "$SWIFT_DIR/Sources/Gelato/Info.plist" "$APP_DIR/Contents/Info.plist"

# Copy app icon
ICON_PATH="$SWIFT_DIR/Sources/Gelato/Assets/AppIcon.icns"
if [[ -f "$ICON_PATH" ]]; then
...
if [[ -n "${CODESIGN_IDENTITY:-}" ]]; then
  ENTITLEMENTS="$SWIFT_DIR/Sources/Gelato/Gelato.entitlements"
